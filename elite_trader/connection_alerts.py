...
def note_binance_error(msg: str | None) -> None:
    ...
    if m:
        ...
        ra = _RETRY_AFTER_RE.search(text)
        if ra:
            try:
                sec = float(ra.group(1))
                _ip_ban_until_ms = max(
                    _ip_ban_until_ms, (time.time() + sec) * 1000.0
                )
            except ValueError:
                pass
        elif "429" in text or "too many requests" in text.lower():
            _ip_ban_until_ms = max(
                _ip_ban_until_ms, (time.time() + 45.0) * 1000.0
            )
    try:
...
def _is_mega9008_live_desk(desk_id: str | None) -> bool:
    d = (desk_id or _desk_instance_id()).strip()
    if d != "9008" and os.getenv("MEGA_INSTANCE_ID", "").strip() != "9008":
        return False
...
def build_alerts(
    *,
    ...
    if (
        ...
        and not _is_mega9008_live_desk(desk_id)
    ):
        alerts.append(
            {
                "level": "warn",
