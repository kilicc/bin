...
_MAX_SYMBOLS      = 50          # açık pozisyon + watchlist
_RECONNECT_MAX    = 10.0        # maks bekleme (sn)
_STALE_SEC        = 4.0         # scalp — mesaj yoksa stale (sn)
_WS_GIVEUP_AFTER  = 5
_WS_SLOW_INTERVAL = 60.0
...
def ingest_mid(coin: str, bid: float, ask: float, ts_ms: int | None = None) -> None:
    ...
    global _last_recv_ms, _connected, _mark_bridge_hits
    try:
        b = float(bid)
        a = float(ask)
        if b <= 0 or a <= 0:
...
def _ingest(payload: dict) -> None:
    global _last_recv_ms, _connected
    ...
    with _lock:
        track = _subscribed
    if track and coin.upper() not in track:
        return
    try:
...
def _on_open(_ws: Any) -> None:
    pass  # _connected sadece mesaj gelince set edilir
...
def _watchdog() -> None:
    while not _stop.is_set():
        ...
        with _lock:
            last = _last_recv_ms
        if _connected and last and (now_ms - last) > _STALE_SEC * 1000:
            _kill_ws()
...
def fast_feed_status() -> dict[str, Any]:
    ...
    ok = n > 0 and lag is not None and lag < _STALE_SEC * 1000
