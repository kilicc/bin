...
(function () {
  ...
  function renderKpis(snap) {
    ...
    const bal = wallet.total_wallet_balance ?? wallet.usdt_balance ?? summ.current_capital ?? summ.equity;
    const margin = wallet.total_margin_balance ?? bal;
    const avail = wallet.available_balance ?? wallet.usdt_available ?? summ.available_balance;
  ...
  function renderClosed(closed) {
    ...
    if (!rows.length) {
      body.innerHTML =
        '<tr><td colspan="11" class="mega-empty-cell">Bu oturumda kapanan işlem yok</td></tr>';
      return;
    }
    body.innerHTML = rows
      .map((c) => {
        const net = Number(c.final_pnl ?? c.net_pnl ?? c.wallet_pnl ?? 0);
        const gross = Number(
          c.exchange_realized_pnl ?? c.pnl_gross_usd ?? c.pnl_usd ?? net
        );
        const fees = Number(
          c.total_fees ?? c.exit_fee ?? Math.max(0, gross - net)
        );
        ...
        const sideCls = side === "SHORT" ? "side-short" : "side-long";
        const src = c.backfilled ? " · sync" : c.fee_source === "binance_api" ? " · API" : "";
        return (
          "<tr>" +
          "<td><strong>" +
          ...
          "</td>" +
          "<td class='mono'>" +
          (entryPx > 0 ? entryPx.toPrecision(6) : "—") +
          "</td>" +
          "<td class='mono'>" +
          (exitPx > 0 ? exitPx.toPrecision(6) : "—") +
          "</td>" +
          '<td class="' +
          pnlClass(gross) +
          '">' +
