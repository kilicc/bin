...
(function () {
  ...
  function mergeConn(hb, live) {
    ...
    const lc = live || {};
    let apiOk = null;
    if (state.snap && state.snap.live) {
      if (hc.api_ok != null) apiOk = !!hc.api_ok;
      else if (lc.api_ok != null) apiOk = !!lc.api_ok;
    } else if (lc.fetch_failed && lc.api_ok == null) {
  ...
  function isBinanceApiLive(conn) {
    if (state.snap && state.snap.live && openPositionCount() > 0) {
      const rows = state.open || [];
      const maxAge = maxAgeMsFromRows(rows);
      const upnlOk = rows.every((p) => liveUpnl(p) != null);
      if (upnlOk && maxAge > 0 && maxAge < PANEL_CONN_FRESH_MS) return true;
    }
    if (!conn) return null;
  ...
  async function runMegaFetch(forceFull) {
    ...
    try {
      ...
      snapLat = Math.round(performance.now() - snapT0);
      if (snapRes.ok) {
        try {
      ...
      const lat = snapLat != null ? snapLat : Math.round(performance.now() - t0);
  ...
  async function init() {
    try {
      ...
      if (g.EliteConnectionBanner && g.EliteConnectionBanner.start) {
        g.EliteConnectionBanner.start();
      }
      startConnPoll();
      tickClock();
