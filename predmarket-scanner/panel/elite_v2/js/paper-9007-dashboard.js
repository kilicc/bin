...
(function () {
  ...
  const OVERLAY_MAX_MS = 3500;
  let lastFullPollTs = Date.now();
  let pollTick = 0;
  let pollTimer = null;
  let tickTimer = null;
  let tickSeq = 0;
  let lastTickLatencyMs = null;
  ...
  const state = { snap: null, selectedIdx: 0, open: [], lastHeroKey: "", hubMarks: null, hubMeta: null };

  const CLOSED_SEEN_KEY = "mega9007_closed_seen";
  const CLOSED_SEED_KEY = "mega9007_closed_seeded";
  const closedNotify = { seen: loadClosedSeenSet() };

  function loadClosedSeenSet() {
    try {
      const raw = sessionStorage.getItem(CLOSED_SEEN_KEY);
      if (raw) return new Set(JSON.parse(raw));
    } catch (_) {}
    return new Set();
  }

  function saveClosedSeenSet(seen) {
    try {
      sessionStorage.setItem(CLOSED_SEEN_KEY, JSON.stringify([...seen]));
    } catch (_) {}
  }

  function closedNotifyKey(c) {
    const oid = String(c.exchange_close_order_id || "").trim();
    if (oid) return "oid:" + oid;
    return "id:" + String(c.id || "") + ":" + String(c.symbol || "");
  }

  function setClosedUnreadBadge(n) {
    const el = document.getElementById("closed-unread-badge");
  ...
  function ackClosedNotifications(closed) {
    const rows = dedupeClosedRows(closed || []);
    rows.forEach((r) => closedNotify.seen.add(closedNotifyKey(r)));
    saveClosedSeenSet(closedNotify.seen);
    sessionStorage.setItem(CLOSED_SEED_KEY, "1");
    setClosedUnreadBadge(0);
  }

  function updateClosedUnread(closed) {
    const rows = dedupeClosedRows(closed || []);
    if (sessionStorage.getItem(CLOSED_SEED_KEY) !== "1") {
      rows.forEach((r) => closedNotify.seen.add(closedNotifyKey(r)));
      saveClosedSeenSet(closedNotify.seen);
      sessionStorage.setItem(CLOSED_SEED_KEY, "1");
      setClosedUnreadBadge(0);
    ...
    let unread = 0;
    rows.forEach((r) => {
      if (!closedNotify.seen.has(closedNotifyKey(r))) unread++;
    });
    setClosedUnreadBadge(unread);
  }

  function bindClosedTabNotify() {
    const section = document.getElementById("mega-closed-section");
    const tab = document.getElementById("tab-closed");
    if (tab) {
      tab.addEventListener("click", () => {
        window.setTimeout(() => ackClosedNotifications((state.snap && state.snap.closed) || []), 100);
      });
    }
        ...
        (entries) => {
          entries.forEach((e) => {
            if (e.isIntersecting && e.intersectionRatio >= 0.22) {
              ackClosedNotifications((state.snap && state.snap.closed) || []);
            }
          });
        },
        { threshold: [0.22, 0.45] }
  ...
  function renderHealthStrip(data) {
    ...
    const cs = data.close_sync || {};
    const lab = data.lab || {};
    const llm = data.llm || {};
    const pills = [
      {
        lbl: "Control",
        val: ctrl.state || ctrl.error || "—",
        tone: ctrl.state === "RUNNING" ? "ok" : ctrl.state === "PAUSED" ? "slow" : "bad",
      },
      {
        lbl: "CloseSync",
        val: cs.last_error ? "ERR" : cs.alive ? "OK" : "—",
        tone: cs.last_error ? "bad" : cs.alive ? "ok" : "off",
      },
      {
        lbl: "Lab LLM",
        val: lab.inflight ? lab.inflight + "↑" : lab.last_latency_ms ? lab.last_latency_ms + "ms" : llm.ok ? "OK" : "—",
        tone: healthTone(llm.ok, lab.error),
      },
    ];
  ...
  async function fetchHealthStrip() {
    try {
  ...
  function liveUpnl(p) {
    ...
    if (ex.unRealizedProfit != null && ex.unRealizedProfit !== "") {
      const binance = Number(ex.unRealizedProfit);
      if (Number.isFinite(binance)) return binance;
    }
  ...
  function createOpenRow(p, i, selected) {
    ...
    tr.innerHTML =
      ...
      "<td class='cell-close'><button type='button' class='btn mega-btn-danger btn-close-pos' data-id='" +
      String(p.id || "") +
      "'>Kapat</button></td>";
  ...
  function renderExchangeSyncBar(snap) {
    const el = document.getElementById("exchange-sync-bar");
    if (!el) return;
    const ex = (snap && snap.exchange_account) || {};
    const w = (snap && snap.wallet) || {};
    const equity = ex.equity_net ?? w.total_margin_balance ?? w.total_wallet_balance;
    if (equity == null && !ex.closed_count) {
      el.classList.add("hidden");
      return;
    }
    el.classList.remove("hidden");
    const parts = [
      "◆ Demo Binance (senkron · net)",
      "Bakiye " + fmtUsd(ex.equity_net ?? equity),
      "Kullanılabilir " + fmtUsd(ex.available_net ?? w.available_balance),
      "uPnL " + fmtUsd(ex.unrealized_net ?? w.total_unrealized_pnl),
      (ex.open_count ?? (snap.open || []).length) + " açık",
      (ex.closed_count ?? (snap.closed || []).length) + " kapalı",
      "Realize net " + fmtUsd(ex.realized_net_settled ?? ex.realized_net),
      "Oturum Δ " + fmtUsd(ex.wallet_session_pnl),
      "Komisyon " + fmtUsd(ex.total_fees),
    ];
    if (ex.closed_settled_count != null && ex.closed_settled_count > 0) {
      parts.push(ex.closed_settled_count + " borsa doğrulanmış");
    }
    if (ex.api_base || w.api_base) {
      parts.push(String(ex.api_base || w.api_base).replace("https://", ""));
    }
  ...
  function renderKpis(snap) {
    ...
    const bal =
      ...
      wallet.usdt_balance ??
      summ.current_capital ??
      summ.equity;
    const margin = ex.equity_net ?? wallet.total_margin_balance ?? bal;
    const avail =
      ex.available_net ??
      wallet.available_balance ??
      wallet.usdt_available ??
      summ.available_balance;
    const closedDeduped = dedupeClosedRows(snap.closed || []);
    let sessionPnl = ex.wallet_session_pnl ?? summ.wallet_session_pnl ?? summ.session_pnl;
    if (sessionPnl == null || !Number.isFinite(Number(sessionPnl))) {
      sessionPnl = closedDeduped.reduce((acc, c) => {
        const n = Number(c.wallet_pnl ?? c.final_pnl ?? c.net_pnl ?? c.pnl_usd ?? 0);
        return acc + (Number.isFinite(n) ? n : 0);
      }, 0);
    } else {
      sessionPnl = Number(sessionPnl);
    }
    const settledRealized = ex.realized_net_settled ?? summ.realized_pnl_settled ?? summ.realized_pnl;
    ...
    const unreal = ex.unrealized_net ?? wallet.total_unrealized_pnl ?? summ.unrealized_pnl;
    ...
    setText("kpi-balance-sub", isLive ? balSub.join(" · ") : "API bekleniyor…");
    ...
    pnlSub.push("Realize (borsa) " + fmtUsd(settledRealized));
    if (anchor != null && margin != null) {
      pnlSub.push("Cüzdan Δ " + fmtUsd(Number(margin) - Number(anchor)));
    }
    if (
    ...
    setText("kpi-pnl-sub", (ex.label ? ex.label + " · " : "") + (pnlSub.join(" · ") || "Kapalı işlemler toplamı (net)"));
    ...
    setText("kpi-open-sub", "MEGA demo · stake $1000 · lev 10x" + (queue ? " · kuyruk " + queue : ""));
  ...
  function renderVolChips(vol, regime) {
    ...
    el.innerHTML = coins
      .map((c) => {
        ...
        const ch = Number(c.change_pct);
        const chTxt = Number.isFinite(ch) ? ch.toFixed(3) + "%" : "";
        ...
        return (
          ...
          '"><span class="sym">' +
          sym +
          '</span> <span class="live-px mono">—</span> ' +
          chTxt +
          vrTxt +
          ' <span class="tier">' +
          tier +
      ...
      .join("");
    if (state.hubMarks) applyHubMarks({ marks: state.hubMarks, hub: state.hubMeta || {} });
  }
  ...
  function renderOpenTable(openList) {
    ...
    if (!rows.length) {
      state.lastHeroKey = "";
      body.innerHTML =
        '<tr><td colspan="10" class="mega-empty-cell">Henüz açık sim pozisyon yok</td></tr>';
  ...
  function bindOpenTable() {
    ...
    body.addEventListener("click", (ev) => {
      const closeBtn = ev.target.closest(".btn-close-pos");
      if (closeBtn) {
        ev.stopPropagation();
        const pid = Number(closeBtn.dataset.id);
        if (Number.isFinite(pid)) closePosition(pid);
        return;
      }
  ...
  function fmtTrExitRow(c) {
    const u = window.ClosedTradeUtils;
    return u ? u.fmtTrExit(c) : (c.exit_time_str || "—");
  }
  ...
  function closedSourceLabel(c) {
    if (c.exchange_settled || c.income_settled) return "API";
    if (c.backfilled || c.sync_source === "exchange_userTrades") return "sync";
    if (c.fee_source === "binance_api") return "API";
    return "local";
  }

  function fmtClosedPx(px) {
    const n = Number(px);
  ...
  function renderClosed(closed) {
    ...
    if (!body) return;
    const rows = sortClosedForDisplay(closed).slice(0, 80);
    if (pill) pill.textContent = String(rows.length);
    if (!rows.length) {
      body.innerHTML =
        '<tr><td colspan="13" class="mega-empty-cell">Bu oturumda kapanan işlem yok</td></tr>';
      updateClosedUnread(closed);
      return;
    }
    body.innerHTML = rows
      .map((c) => {
        const net = Number(c.wallet_pnl ?? c.final_pnl ?? c.net_pnl ?? 0);
        const fees = Number(c.total_fees ?? c.exit_fee ?? 0);
        const gross = Number(
          c.exchange_realized_pnl ?? c.pnl_gross_usd ?? c.pnl_usd ?? net + fees
        );
        const stake = Number(c.stake_usd || 0);
        const netPct = Number(
          c.net_pnl_pct ?? (stake > 0 ? (net / stake) * 100 : 0)
        );
        ...
        const srcCls =
          src === "API" ? "src-api" : src === "sync" ? "src-sync" : "src-local";
        return (
          "<tr>" +
          "<td><strong>" +
          esc((c.symbol || "").replace("USDT", "")) +
          "</strong></td>" +
          '<td class="' +
          sideCls +
          '">' +
          ...
          "</td>" +
          "<td class='mono cell-num'>" +
          fmtClosedPx(c.entry_price) +
          "</td>" +
          "<td class='mono cell-num'>" +
          fmtClosedPx(c.exit_price) +
          "</td>" +
          '<td class="cell-num ' +
          pnlClass(gross) +
          '">' +
          ...
          "</td>" +
          "<td class='mono cell-src " +
          srcCls +
          "'>" +
          esc(src) +
          "</td>" +
          "<td class='cell-reason' title='" +
          esc(reason) +
          "'>" +
          esc(reason) +
          "</td>" +
          "<td class='mono cell-dt'>" +
          esc(fmtTrExitRow(c)) +
          "</td>" +
          "</tr>"
        );
  ...
  function setTickLatency(ms) {
    const valEl = document.getElementById("poll-latency");
    if (!valEl) return;
    const n = Number(ms);
    if (Number.isFinite(n)) lastTickLatencyMs = n;
    const tick = Number.isFinite(lastTickLatencyMs) ? Math.round(lastTickLatencyMs) + "ms" : "—";
    setTextNode(valEl, tick);
    const wrap = document.getElementById("snapshot-latency");
    if (!wrap) return;
    const tone =
      Number.isFinite(lastTickLatencyMs) && lastTickLatencyMs >= 250
        ? lastTickLatencyMs >= 600
          ? "bad"
          : "slow"
        : "ok";
  ...
  function applyHubMarks(data) {
    const marks = data && data.marks;
    if (!marks || typeof marks !== "object") return;
    state.hubMarks = marks;
    const hub = (data && data.hub) || {};
    state.hubMeta = hub;
    if (hub.mark_lag_ms != null) {
      const spec = MOTOR_PILLS.find((p) => p.id === "mark");
      const strip = ensureMotorStrip();
      const pill = strip && strip.querySelector('[data-motor-id="mark"]');
      if (pill && spec) {
        const val = pill.querySelector(".val");
        if (val) setTextNode(val, fmtMotorMs(hub.mark_lag_ms));
        const tone = motorPillTone(hub.mark_lag_ms, spec);
        const next = "mega-motor-pill " + tone;
        if (pill.className !== next) pill.className = next;
      }
    }
    document.querySelectorAll("[data-coin]").forEach((node) => {
      const coin = node.dataset.coin;
      if (!coin) return;
      const px = Number(marks[coin] || marks[coin + "USDT"]);
      if (!Number.isFinite(px) || px <= 0) return;
      const el = node.querySelector(".live-px");
      if (el) setTextNode(el, fmtPx(px));
    });
  }
  ...
  function setSnapshotLatency(ms) {
    ...
    if (Number.isFinite(n)) {
      wrap.dataset.snapMs = String(Math.round(n));
    }
  }
  ...
  function scheduleOverlayFallback() {
    ...
    overlayFallbackTimer = setTimeout(function () {
      ...
      setText(
        "kpi-balance-sub",
        "Önbellek modu — REST koruması aktif; hub/tick ile güncelleniyor…"
      );
    }, OVERLAY_MAX_MS);
  }
  ...
  async function runMegaFetch(forceFull) {
    ...
    let snapRes = null;
    try {
      const snapUrl = "/api/paper/" + MODE_ID + "/snapshot?light=" + (full ? "0" : "1");
      const hbPromise = fetchWithTimeout("/api/heartbeat", HEARTBEAT_TIMEOUT_MS).catch(
        () => null
      );
      const [snapResRaw, hbRes] = await Promise.all([
        fetchWithTimeout(snapUrl, SNAPSHOT_TIMEOUT_MS),
        hbPromise,
      ]);
      snapRes = snapResRaw;
      if (snapRes && snapRes.ok) snap = await snapRes.json();
      else if (snapRes && (snapRes.status === 401 || snapRes.status === 403)) {
        setText(
          "kpi-balance-sub",
      ...
      if (hbRes && hbRes.ok) hb = await hbRes.json();
    } catch (_) {
      setText(
        "kpi-balance-sub",
        "API yavaş veya REST koruması — önbellek/tick ile devam ediliyor"
      );
    }
    ...
    if (snap && snap.ok) {
      try {
        ...
        renderKpis(snap);
        renderExchangeSyncBar(snap);
        renderScan(snap.scan);
        renderOpenTable(snap.open);
        renderClosed(dedupeClosedRows(snap.closed || []));
        schedulePoll((snap.open || []).length > 0);
      } catch (renderErr) {
        console.error("MEGA panel render", renderErr);
        setText("kpi-balance-sub", "Panel render hatası — yenile deneyin");
      }
    } else if (!snap) {
  ...
  async function closePosition(id) {
    try {
      const r = await fetch("/api/mega/positions/" + id + "/close", { method: "POST" });
      const j = await r.json();
      appendControlLog("close id=" + id + " ok=" + j.ok);
      fetchMega(true);
    } catch (e) {
      appendControlLog("close failed id=" + id);
    }
  }
  ...
  function applyControlUi(st) {
    ...
    if (restartBtn) restartBtn.disabled = controlState.state === "RESTARTING";
    if (hint) {
      if (pausing) {
        hint.textContent =
          "Zararda " + (st.pending_losers || 0) + " pozisyon breakeven bekliyor";
      } else if (st.pause_warn) {
        hint.textContent = "Pause süresi uzadı — Restart önerilir";
      } else {
        hint.textContent = "";
      }
    }
  ...
  async function fetchControlStatus() {
    try {
      const r = await fetch("/api/mega/control/status");
      if (!r.ok) return;
  ...
  function bindControlButtons() {
    ...
    if (pauseBtn) {
      pauseBtn.addEventListener("click", async () => {
        appendControlLog("pause requested");
        const r = await fetch("/api/mega/control/pause", { method: "POST" });
        applyControlUi(await r.json());
      });
    }
    if (startBtn) {
      startBtn.addEventListener("click", async () => {
        appendControlLog("start requested");
        const r = await fetch("/api/mega/control/start", { method: "POST" });
        applyControlUi(await r.json());
      });
    }
    if (restartBtn) {
      restartBtn.addEventListener("click", async () => {
        if (!confirm("Tüm pozisyonlar kapatılır ve oturum arşivlenir. Emin misiniz?")) return;
        appendControlLog("restart requested");
        restartBtn.disabled = true;
        const r = await fetch("/api/mega/control/restart", { method: "POST" });
        applyControlUi(await r.json());
        fetchMega(true);
      });
    }
  ...
  async function fetchLabSim() {
    try {
      const r = await fetch("/api/lab/sim/counterfactual");
      const j = await r.json();
      if (!j.ok) return;
      const panel = document.getElementById("lab-sim-panel");
      const body = document.getElementById("lab-sim-body");
      if (!panel || !body) return;
      panel.classList.remove("hidden");
      body.innerHTML = (j.presets || [])
        .map(function (row) {
          return (
            "<tr><td>" +
            row.label +
  ...
  function tickHeroLive() {
    ...
    if (selected) {
      ...
      const unreal = liveUpnl(selected);
      const upnlEl = document.getElementById("hero-upnl");
      if (upnlEl) {
        setTextNode(upnlEl, fmtUsd(unreal));
        syncToneClass(upnlEl, "val", pnlClass(unreal));
      }
      const tpPct = Math.round(tpProgress(selected) * 100);
      setText("hero-tp-pct", tpPct + "%");
      const bar = document.getElementById("hero-tp-bar");
      if (bar) bar.style.width = tpPct + "%";
    }
  ...
  async function fetchTicks() {
    ...
    try {
      ...
      if (seq !== tickSeq || !res.ok) return;
      const data = await res.json();
      if (seq !== tickSeq || !data || !data.ok) return;
      setTickLatency(performance.now() - t0);
      if (data.ts) {
        setText("footer-ts", String(data.ts).slice(0, 19).replace("T", " "));
      }
      applyHubMarks(data);
      if (data.open && data.open.length && state.open && state.open.length) {
  ...
  async function init() {
    ...
    startLiveLogPoll();
    fetchLabSim();
    setInterval(fetchLabSim, 12000);
    fetchHealthStrip();
    setInterval(fetchHealthStrip, 5000);
