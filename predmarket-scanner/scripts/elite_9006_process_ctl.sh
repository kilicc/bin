#!/usr/bin/env bash
# Elite 9006 (MEGA) — tek örnek, port temizliği, güvenli start/stop/restart
set -euo pipefail
...
PORT="${ELITE_9006_PORT:-9006}"
PID_DIR=".pids"
PID_FILE="${PID_DIR}/binance_elite_mega_9006.pid"
START_LOCK="${PID_DIR}/elite_9006_start.lock"
PROC_PATTERN="binance_elite_pro_9006.py"
RUN_SH="${ROOT}/run_binance_elite_mega_9006.sh"
...
elite_9006_stop() {
  ...
  echo "  ⏹ Elite 9006 (MEGA) durduruluyor (port ${PORT})..."
  ...
  rm -f "${START_LOCK}" "${PID_DIR}/elite_instance_9006.lock"
...
elite_9006_status() {
  ...
  echo "Elite 9006 (MEGA) — port ${PORT}"
  echo "  PID dosyası: ${pid_file_pid:-—}"
  echo "  LISTEN: ${listen:-—}"
  echo "  binance_elite_pro_9006: ${proc:-—}"
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  case "${cmd}" in
    stop) elite_9006_stop 0 ;;
    force-stop) elite_9006_stop 1 ;;
    start) elite_9006_start_locked ;;
    restart) elite_9006_restart ;;
    status) elite_9006_status ;;
    *)
