#!/usr/bin/env bash
# Elite 9005 — tek örnek, port temizliği, güvenli start/stop/restart
set -euo pipefail
...
START_LOCK="${PID_DIR}/elite_9005_start.lock"
PROC_PATTERN="binance_elite_pro_9005.py"
RUN_SH="${ROOT}/run_binance_elite_8300_9005.sh"
...
elite_9005_stop() {
  ...
  echo "  ⏹ Elite 9005 durduruluyor (port ${PORT})..."
  ...
  # 2) İsimle tüm örnekler (zombie / eski restart kalıntıları)
  local pids
  pids=$(_proc_pids)
  if [[ -n "${pids}" ]]; then
    if [[ "${force}" == "1" ]]; then
  ...
  # 3) Port LISTEN (yalnızca sunucu — tarayıcı bağlantılarına dokunma)
  local listen
  listen=$(_listen_pids)
  if [[ -n "${listen}" ]]; then
    if [[ "${force}" == "1" ]]; then
...
elite_9005_status() {
  ...
  echo "Elite 9005 — port ${PORT}"
  echo "  PID dosyası: ${pid_file_pid:-—}"
  echo "  LISTEN: ${listen:-—}"
  echo "  binance_elite_pro_9005: ${proc:-—}"
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  case "${cmd}" in
    stop) elite_9005_stop 0 ;;
    force-stop) elite_9005_stop 1 ;;
    start) elite_9005_start_locked ;;
    restart) elite_9005_restart ;;
    status) elite_9005_status ;;
    *)
