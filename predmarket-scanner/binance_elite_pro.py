...
def _purge_paper_port_api_keys() -> None:
    """9005/9006 — .env anahtarları olsa bile imzalı REST mümkün olmasın."""
    for key in _PAPER_PURGE_KEYS:
...
def _lock_9005_paper_env() -> None:
    ...
    for key, val in (
        ...
        ("ELITE_BINANCE_DATA_HUB", "1"),
        ("ELITE_BINANCE_HUB_URL", "http://127.0.0.1:9007"),
        ("BN_FUT_MARK_WS", "0"),
        ("BN_FUT_FAST_WS", "0"),
    ):
        os.environ[key] = val
    _purge_paper_port_api_keys()


def _lock_9006_paper_env() -> None:
    """9006 — MEGA paper sim (hub tüketici)."""
...
def _lock_9006_demo_live_env() -> None:
    """9006 — demo-fapi canlı MEGA (9005/9007 kapalıyken kendi WS+REST)."""
    if not _is_9006_process():
        return
    for key, val in (
        ...
        os.environ[key] = val
    # MEGA_* anahtarları .env.mega_9006 — global BINANCE_* ile karışmasın
    for mega_key in ("BINANCE_API_KEY", "BINANCE_API_SECRET", "BINANCE_FUTURES_API_SECRET"):
        mk = os.getenv(f"MEGA_{mega_key}", "").strip()
        if mk:
            os.environ[mega_key] = mk
            os.environ[f"MEGA_{mega_key}"] = mk
...
def _reinit_clients_after_9006_env() -> None:
    """9006 demo canlı — client + MEGA singleton yenile."""
    if not _is_9006_process():
        return
    if not _env_bool("MEGA_LIVE_ORDERS", False):
        return
    _lock_9006_demo_live_env()
    _reinit_binance_client_from_env()
    try:
        from elite_trader import mega_live as ml
...
if _is_9005_demo():
    ...
    if _is_9007_process():
        ...
        if _env_bool("MEGA_LIVE_ORDERS", False):
            _lock_9006_demo_live_env()
        else:
...
if ELITE_PORT == 9005:
    ...
    if ELITE_PORT == 9007:
        ...
        if _env_bool("MEGA_LIVE_ORDERS", False):
            _lock_9006_demo_live_env()
            _reinit_clients_after_9006_env()
        else:
            _lock_9006_paper_env()
            _reinit_binance_client_from_env()
        try:
...
def _restore_closed_from_db() -> None:
    ...
    if not _elite_state_enabled():
        return
    try:
        from elite_pro_state import bootstrap_closed, load_closed, ensure_cutover_closed_merged

        ensure_cutover_closed_merged(active_execution_mode())
...
def _rest_price_refresh_loop() -> None:
    ...
    while not _rest_refresh_stop.is_set():
        try:
            from elite_trader.binance_data_hub import hub_consumer_mode, sync_consumer_marks

            if hub_consumer_mode():
                marks = sync_consumer_marks()
                if marks:
                    now = time.time()
                    with _price_cache_lock:
                        _price_cache_prev.update(_price_cache)
                        _price_cache.update(marks)
                        _price_cache_ts = now
                    _trim_global_price_caches()
                _rest_refresh_stop.wait(
                    max(0.12, _env_float("ELITE_BINANCE_HUB_CONSUMER_SEC", "0.15"))
                )
                continue
        except ImportError:
            pass
...
def _bulk_prices_cached() -> dict[str, float]:
    ...
    hub_px = _hub_consumer_prices_bulk()
    if hub_px:
        with _price_cache_lock:
            _price_cache.update(hub_px)
            _price_cache_ts = now
...
def _hub_consumer_prices_bulk() -> dict[str, float]:
    """9005/9006 — 9007 hub (disk → HTTP → stale → shadow WS)."""
    try:
        from elite_trader.binance_data_hub import consumer_marks_bulk, hub_consumer_mode

        if not hub_consumer_mode():
            return {}
        marks = consumer_marks_bulk(allow_stale=True)
        return marks if len(marks) >= 5 else {}
    except Exception:
        return {}
...
def get_current_capital() -> float:
    ...
    if _is_paper_only_port():
        try:
            ...
            return paper_book_equity_floor(mid)
        except Exception:
            pass
...
def _mega_status_client():
    """Panel bağlantı durumu — paper fallback olsa da MEGA desk client'ı."""
    try:
        from elite_trader.mega_live import mega_live_enabled, get_mega_client
...
def _api_stale_sec() -> float:
    return max(12.0, _env_float("ELITE_API_STALE_SEC", 35.0))


def _api_healthy() -> bool:
    """Canlı desk — son başarılı Binance REST/ping (auth/ban hariç)."""
    try:
        ...
        pass
    mc = _mega_status_client() if _is_mega_primary_process() else None
    if mc and not mc.paper:
        if mc._auth_error:
            return False
        try:
            from elite_trader.mega_live import (
                ...
                _mega_positions_list_age_sec,
            )

            from elite_trader.mega_live import _mega_panel_exchange_only

            from elite_trader.mega_live import mega_open_coins

            open_n = len(mega_open_coins())
            ping_age = last_ok_age_sec()
            risk_age = _mega_positions_list_age_sec()
            ping_ok = ping_age is not None and ping_age <= _api_stale_sec()
            risk_cap = (
                max(5.0, _mega_panel_pos_risk_max_age_sec() * 3)
                if _mega_panel_exchange_only()
                else max(15.0, _env_float("MEGA_HUB_POS_FRESH_MS", 120_000) / 1000.0)
            )
            risk_ok = open_n == 0 or risk_age <= risk_cap
            # #region agent log
            try:
                from elite_trader.mega_live import _dbg6fe627

                _dbg6fe627(
                    "H2",
                    "binance_elite_pro.py:_api_healthy",
                    "mega_api_health",
                    {
                        "open_n": open_n,
                        "ping_age": ping_age,
                        ...
                        "ping_ok": ping_ok,
                        "risk_ok": risk_ok,
                        "result": ping_ok and risk_ok,
                    },
                )
            except Exception:
                pass
            # #endregion
            if ping_ok and risk_ok:
                return True
        except Exception:
            pass
        return False
    if LIVE_ORDERS and not client.paper:
        if client._auth_error:
            return False
        try:
            from elite_trader.connection_alerts import last_ok_age_sec

            age = last_ok_age_sec()
            if age is not None and age <= _api_stale_sec():
                return True
        except Exception:
            pass
        try:
...
def _execute_exchange_market_close(
    ...
    *,
    ...
    try:
        ...
        if not is_order_timeout_error(exc):
            raise
        print(
            f"  ⚠ Borsa kapanış timeout {pos['symbol']} (-1007) — "
            f"positionRisk/userTrades doğrulanıyor"
        )
        exchange_settled, close_order_id = recover_close_after_order_timeout(
            client, pos, settle_wait_sec=0.5
        )
        if exchange_settled:
            close_exec["order_ack_ms"] = round(
                (time.perf_counter() - t_order0) * 1000.0, 2
            )
            close_exec["close_timeout_recovered"] = True
            close_exec["settle_ms"] = 0.0
            print(
                f"  ✓ Borsa kapanış {pos['symbol']} {exit_reason} "
                f"(timeout kurtarıldı) order={close_order_id or '—'}"
            )
            return exchange_settled, close_order_id or None, close_exec
        if _exchange_position_row(pos["symbol"], pos["side"], force_fresh=True):
            print(
                f"  ⛔ Borsa kapanış timeout {pos['symbol']}: "
                f"pozisyon hâlâ açık — tekrar MARKET gönderilmedi"
            )
            raise
        exchange_settled = _settle_close_with_retry(pos, close_order_id or None)
        if exchange_settled:
            close_exec["close_timeout_recovered"] = True
            return exchange_settled, close_order_id or None, close_exec
        raise
        ...
        if ep_row and qty > 0:
            try:
                ...
                if is_order_timeout_error(exc):
                    ...
                    if exchange_settled:
                        pass
                    elif not _exchange_position_row(
                        pos["symbol"], pos["side"], force_fresh=True
                    ):
                        return _drop_local_exchange_position(i, pos, exit_reason)
                    else:
                        print(
                            f"  ⛔ Borsa kapanış başarısız {pos['symbol']}: "
                            f"timeout, pozisyon açık"
                        )
                        return False
                elif "-2022" in err or "ReduceOnly" in err:
...
def _prices_for_coins(
    ...
    try:
        ...
        if hub_consumer_mode():
            hub_px = hub_prices_for_coins(coins)
            if hub_px:
                return hub_px
    except ImportError:
        pass
    ...
    try:
        ...
        ff = fast_feed_status()
        if ff.get("enabled") and not ff.get("connected"):
            use_fast = False
    except Exception:
        pass
    if use_fast:
...
def _apply_position_price_tick(pos: dict[str, Any], bulk: dict[str, float]) -> None:
    ...
    if not bulk_price or float(bulk_price) <= 0:
        return
    px = float(bulk_price)
    prev = float(pos.get("current_price") or 0)
    hist = pos.setdefault("price_history", [])
    times = pos.setdefault("time_history", [])
    ref = prev if prev > 0 else (float(hist[-1]) if hist else px)
    min_move = max(abs(ref) * 1e-5, 1e-8)
    if ref > 0 and abs(px - ref) < min_move:
        return
    if len(hist) >= 2 and abs(px - hist[-2]) < min_move and abs(hist[-1] - hist[-2]) >= min_move:
        return
    hist.append(px)
    times.append(_position_time_label())
    if len(hist) > 60:
...
def update_open_positions_fast() -> float:
    ...
    if mega_primary:
        ...
        if not bulk:
            mega_deadline = t0 + (
                min(0.15, budget_ms / 1000.0) if sim_only else min(0.012, budget_ms / 1000.0)
            )
            bulk = _prices_for_coins(mega_coins, deadline=mega_deadline, max_age_ms=max_age)
        if not bulk:
            bulk = _prices_from_local_ticks(mega_coins)
        try:
            from elite_trader.mega_live import touch_mega_open_from_cache

            touch_mega_open_from_cache(bulk or {})
        except Exception as exc:
...
def _heartbeat_payload() -> dict[str, Any]:
    ...
    try:
        ...
        mc = _mega_status_client() if _is_mega_primary_process() else None
        active = mc if mc else client
        age = last_ok_age_sec()
        risk_age: float | None = None
        try:
            from elite_trader.mega_live import _mega_positions_list_age_sec

            risk_age = round(_mega_positions_list_age_sec(), 2)
        except Exception:
            pass
        conn = {
            "api_ok": bool(_api_healthy()),
            "api_paper": bool(active.paper),
            "auth_error": active._auth_error,
            "ip_ban": ip_ban_active(),
            "last_ok_ago_sec": age,
            "position_risk_age_sec": risk_age,
            "stale_sec": _api_stale_sec(),
        }
    except Exception:
        pass
    return {
    ...
    try:
        ...
        if ff.get("enabled"):
            if not ff.get("connected"):
                ...
                lag = int(ff.get("ws_lag_ms") or ff.get("lag_ms") or 0)
                bt_max = _env_int("ELITE_BOOKTICKER_MAX_LAG_MS", 8000)
                if lag > bt_max:
                    fast_stale = True
    except Exception:
        pass
...
def _connection_keeper_tick() -> None:
    ...
    if LIVE_ORDERS or _is_mega_primary_process():
        ping_client = client
        if _is_mega_primary_process():
            mc = _mega_status_client()
            if mc and not mc.paper:
                ping_client = mc
        if ping_client.paper:
        ...
        if tick % 30 == 0:
            ...
            if len(watchlist) < 50 and tick % 60 == 0:
                try:
                    ...
                    pass
            wl_n = len(watchlist) or len(tradable_symbols) or 0
            tick_n = int(ss.get("symbols_with_ticks") or 0)
            print(
                f"  📡 Pipeline: evren={tick_n}/{wl_n} "
                f"aday={_last_motor_candidates_n} eval={_last_motor_eval_n} "
                f"sinyal={len(signals)} paper_açık={paper_open}"
            )
            if wl_n >= 20 and tick_n < max(5, int(wl_n * 0.12)):
                print(
                    f"  ⛔ Pipeline uyarı: evren fiyatı eksik ({tick_n}/{wl_n}) — "
                    "hub/WS kopuk, pozisyon açılmaz"
                )
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            mc_btc = get_mega_client()
            if mc_btc and not mc_btc.paper:
                schedule_btc_refresh(mc_btc)
            ready, _ready_tag = btc_context_ready(price_history, binance_client=mc_btc)
            if not ready:
                return 0
            bear, _bear_tag = btc_bearish(price_history, binance_client=mc_btc)
        except Exception:
            return 0
        ...
        if bear:
            min_pct = min(
                min_pct,
                _env_float("MEGA_BEAR_VOL_MIN_MOMENTUM_PCT", 0.0005),
            )
        flash_enabled = os.getenv("MEGA_FLASH_REVERSAL_ENABLED", "1").strip().lower() in (
            "1", "true", "yes"
        )
        flash_pump_enabled = os.getenv("MEGA_FLASH_PUMP_ENABLED", "1").strip().lower() in (
            "1", "true", "yes"
        )
        # Flash Reversal: açık pozisyonu olmayan sembolleri önce tara
        try:
            from elite_trader.mega_live import _mega_positions as _cur_open_pos
            _open_syms = {str(p.get("symbol", "")).upper() for p in _cur_open_pos}
            symbols = sorted(symbols, key=lambda s: (s.upper() in _open_syms, s))
        except Exception:
            pass
        n = 0
        btc_cascade_st: dict | None = None
        try:
            from elite_trader.btc_flash_cascade import (
                build_cascade_capitulation_long,
                build_cascade_direct_btc_long,
                build_cascade_direct_btc_short,
                detect_btc_cascade,
                sort_cascade_symbols,
            )
            from elite_trader.mode_profiles import get_profile as _cascade_prof

            btc_cascade_st = detect_btc_cascade(price_history)
            px_btc = float(prices.get("BTCUSDT") or prices.get("BTC") or 0)
            if btc_cascade_st and px_btc > 0:
                if str(btc_cascade_st.get("phase")) == "cascade_down":
                    ds = build_cascade_direct_btc_short(px_btc, btc_cascade_st)
                    if ds and process_scan_candidate(ds, px_btc):
                        n += 1
                if str(btc_cascade_st.get("phase")) in ("capitulation", "cascade_down"):
                    prof_c = _cascade_prof("mega") or _cascade_prof("berserk2") or {}
                    cl = build_cascade_capitulation_long(price_history, prof_c)
                    if cl and process_scan_candidate(cl, px_btc):
                        n += 1
            symbols = sort_cascade_symbols(symbols, btc_cascade_st)
        except Exception:
            pass
        for sym in symbols:
            ...
            if fp <= 0:
                continue
            if btc_cascade_st and str(btc_cascade_st.get("phase")) == "cascade_down":
                try:
                    from elite_trader.btc_flash_cascade import build_cascade_follow_short

                    cs = build_cascade_follow_short(
                        sym, fp, price_history, btc_cascade_st
                    )
                    if cs and process_scan_candidate(cs, fp):
                        n += 1
                        continue
                except Exception:
                    pass
            # Bear: önce flash SHORT; LONG flash kapalı
            if flash_pump_enabled and bear:
                try:
                    from elite_trader.berserk2_flash_reversal import (
                        build_flash_pump_reversal_candidate,
                        flash_pump_reversal_enabled,
                    )
                    from elite_trader.mode_profiles import get_profile

                    prof_fp = get_profile("mega") or get_profile("berserk2") or {}
                    if flash_pump_reversal_enabled(prof_fp):
                        fpump = build_flash_pump_reversal_candidate(
                            sym, fp, price_history, prof_fp, rank=0
                        )
                        if fpump:
                            fpump["signal_source"] = "MegaFlashReversal-SHORT"
                            fpump["target_mode"] = "mega"
                            fpump["mega_flash_pump"] = True
                            fpump.pop("berserk2_mover_rank", None)
                            if process_scan_candidate(fpump, fp):
                                n += 1
                                continue
                except Exception:
                    pass
            if flash_enabled and not bear:
                try:
                    from elite_trader.berserk2_flash_reversal import (
                        build_flash_reversal_candidate,
                    )
                    from elite_trader.mode_profiles import get_profile

                    prof_fr = get_profile("mega") or get_profile("berserk2") or {}
                    fr = build_flash_reversal_candidate(
                        sym, fp, price_history, prof_fr, rank=0
                    )
                    if fr:
                        fr["signal_source"] = "MegaFlashReversal-LONG"
                        fr["target_mode"] = "mega"
                        fr["mega_flash_reversal"] = True
                        fr.pop("berserk2_mover_rank", None)
                        if process_scan_candidate(fr, fp):
                            n += 1
                            continue
                except Exception:
                    pass
            if flash_pump_enabled and not bear:
                try:
                    from elite_trader.berserk2_flash_reversal import (
                        build_flash_pump_reversal_candidate,
                        flash_pump_reversal_enabled,
...
def _flash_reversal_watch_for_ui(limit: int = 16) -> list[dict[str, Any]]:
    ...
    try:
        from elite_trader.mega_live import _mega_flash_watch_rows

        rows = _mega_flash_watch_rows(limit=max(limit, 20))
    except Exception:
        rows = []

    _reversal_watch_cache = list(rows)
...
def _closed_dedupe_key(row: dict[str, Any]) -> tuple[Any, ...]:
    ...
    if rid is not None:
        # Reset sonrası aynı id farklı sembol — id tek başına yeterli değil
        return ("id", int(rid), sym, side)
    return (
        "row",
        sym,
        side,
...
def _closed_positions_for_ui() -> list[dict[str, Any]]:
    ...
    if LIVE_ORDERS and not client.paper:
        try:
            ...
            pass
    return _sort_closed_by_exit_desc(out)


def _sort_closed_by_exit_desc(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Kapalı tablo — en yeni çıkış en üstte."""

    def _exit_ts(r: dict[str, Any]) -> float:
        for key in ("exit_time", "exit_time_iso", "exit_time_str", "closed_at_iso"):
            ts = _parse_event_ts(r.get(key))
            if ts > 0:
                return ts
        return 0.0

    return sorted(rows, key=_exit_ts, reverse=True)
...
def delete_closed_trade(
    ...
    *,
    ...
    try:
        ...
        pass
    if mode_id == "mega" or _is_mega_primary_process():
        try:
            from elite_trader.mega_live import _remove_mega_closed_records

            _remove_mega_closed_records(
                position_ids=[tid],
                symbols=[symbol] if symbol else None,
                reason=reason,
            )
        except Exception as exc:
            print(f"  ⚠ MEGA kapalı silme: {exc}")
...
async def lifespan(app: FastAPI):
    ...
    try:
        from elite_trader.binance_data_hub import (
            hub_consumer_mode,
            hub_publisher_mode,
            start_hub_publisher,
        )
        ...
        if hub_publisher_mode():
            start_hub_publisher()
            print("  📡 Binance data hub YAYIN (9007 → 9005/9006 mainnet fiyat)")
        elif hub_consumer_mode():
            from elite_trader.binance_data_hub import start_hub_consumer_resilience

            start_hub_consumer_resilience()
            print("  📡 Binance data hub TÜKETİCİ — disk+HTTP+stale yedek, 9007 paylaşım")
    except Exception as exc:
        print(f"  ⚠ Binance data hub: {exc}")
    try:
    ...
    if not _async_hub_enabled():
        try:
            ...
            pass
    _hub_consumer_boot = False
    try:
        from elite_trader.binance_data_hub import hub_consumer_mode

        _hub_consumer_boot = hub_consumer_mode()
    except ImportError:
        pass
    if not _hub_consumer_boot:
        try:
        ...
        print("  📡 Fiyat önbellek: 9007 hub (client.all_prices atlandı)")
        try:
            from elite_trader.binance_data_hub import consumer_marks_bulk

            _hub_px = consumer_marks_bulk(allow_stale=True)
            if len(_hub_px) >= 5:
                _now0 = time.time()
                with _price_cache_lock:
                    _price_cache_prev.update(_price_cache)
                    _price_cache.update(_hub_px)
                    _price_cache_ts = _now0
                print(f"  ✅ Hub fiyat önbellek: {len(_hub_px)} sembol")
        except Exception:
            pass
    if len(_price_cache) < 5:
        try:
...
def _panel_snap_from_mega_live(*, light: bool = True) -> dict[str, Any] | None:
    ...
    return {
        ...
        "signals": [],
        "approaching_signals": [],
        "approaching_reversal_signals": list((scan.get("flash_watch") or [])[:20]),
        "mega_scan": scan,
        "scanner_status": {
...
def _berserk2_scan_context() -> dict[str, Any]:
    """9005 paper berserk2 — tarama + red özeti."""
    age = None
    ...
    try:
        ...
        rb = reject_summary("berserk2")
        scan_ctx["reject_total"] = rb.get("reject_count")
        scan_ctx["reject_top"] = rb.get("top_reject_reasons")
        scan_ctx["reject_buffer"] = rb
        scan_ctx["recent_rejects"] = rb.get("last_20_rejects")
    except Exception:
        pass
    try:
        from elite_trader.mode_registry import berserk2_paper_only

        scan_ctx["berserk2_paper_only"] = berserk2_paper_only()
    except Exception:
        pass
    return scan_ctx


def _paper_hub_marks_for_coins(coins: list[str]) -> dict[str, float]:
    marks: dict[str, float] = {}
    if not coins:
        return marks
    try:
        from elite_trader.binance_data_hub import hub_prices_for_coins

        marks = hub_prices_for_coins(coins) or {}
    except Exception:
        pass
    if marks:
        return marks
    try:
        from elite_trader.binance_data_hub import fetch_market_feed

        snap = fetch_market_feed()
        raw = ((snap or {}).get("mainnet") or {}).get("marks") or {}
        for coin in coins:
            key = str(coin).upper().replace("USDT", "")
            px = raw.get(key) or raw.get(f"{key}USDT")
            if px and float(px) > 0:
                marks[key] = float(px)
    except Exception:
        pass
...
def _mega_scan_context_meta() -> dict[str, Any]:
    """Panel light snapshot — motor meta only (vol/BTC zaten scan_summary_light'ta)."""
    age = None
    if _last_berserk2_scan_at > 0:
        age = round(time.time() - _last_berserk2_scan_at, 1)
...
def _mega_scan_context() -> dict[str, Any]:
    ...
    try:
        ...
        if mc_btc and not mc_btc.paper:
            schedule_btc_refresh(mc_btc)
        btc_block = btc_context_snapshot(price_history, binance_client=mc_btc)
    except Exception:
        pass
    out["volatility"] = vol_block
    out["btc_context"] = btc_block
    return out


@app.get("/api/paper/{mode_id}/ticks")
def api_paper_mode_ticks(mode_id: str):
    """Paper açık pozisyon mark/uPnL — hafif poll (snapshot yükü yok)."""
    ...
    try:
        if mid == "mega":
            from elite_trader.mega_live import open_ticks_for_ui

            return open_ticks_for_ui()
        book = parallel_engine.get_universe_book(mid)
        open_raw = list(book.get("open") or [])
        coins = [
            str(p.get("symbol") or "").upper().replace("USDT", "") for p in open_raw
        ]
        marks = _paper_hub_marks_for_coins(coins)
        if marks:
            parallel_engine.patch_open_prices(marks)
            book = parallel_engine.get_universe_book(mid)
            open_raw = list(book.get("open") or [])
        open_ui = _enrich_paper_positions_for_ui(open_raw, mid)
        return {
            "ok": True,
            "ts": datetime.utcnow().isoformat() + "Z",
            "open": open_ui,
        ...
        return {"ok": False, "error": str(exc), "open": []}
    ...
    try:
        if mid == "mega":
            ...
            if mega_motor_active():
                ...
                if is_live:
                    sync_info = panel_exchange_sync_meta()
                    if sync_info.get("mismatch"):
                        from elite_trader.mega_live import (
                            panel_exchange_sync,
                            prune_book_positions_not_on_exchange,
                        )

                        only_book = sync_info.get("only_book") or []
                        only_ex = sync_info.get("only_exchange") or []
                        if only_book and not only_ex:
                            prune_book_positions_not_on_exchange()
                            sync_info = panel_exchange_sync_meta()
                        else:
                            sync_info = panel_exchange_sync(force=True)
                snap = mega_snap(light=light_snap)
                book = parallel_engine.get_universe_book(mid)
                open_ui = list(snap.get("open") or [])
                # Canlı demo-fapi: yalnızca mega_live kitabı — parallel paper kitap hayalet üretmesin.
                if not open_ui and not is_live:
                    open_ui = _enrich_paper_positions_for_ui(
                        list(book.get("open") or []), mid
                    )
                elif is_live:
                    pu_open = list(book.get("open") or [])
                    if pu_open and not open_ui:
                        parallel_engine.clear_paper_open_positions(mode_id=mid)
                closed_ui = list(snap.get("closed") or [])
                book_closed = list(book.get("closed") or [])
                if mega_sim_enabled() and not mega_live_enabled():
                    # MEGA sim kapanışları mega_live_closed — parallel kitap ezmesin
                    if book_closed:
                        try:
                            from elite_trader.mega_live import _closed_dedupe_key

                            seen = {_closed_dedupe_key(r) for r in closed_ui}
                            for row in book_closed:
                                k = _closed_dedupe_key(row)
                                if k not in seen:
                                    closed_ui.append(row)
                                    seen.add(k)
                        except Exception:
                            if not closed_ui:
                                closed_ui = book_closed[-100:]
                    try:
                        from elite_trader.mega_live import _mega_panel_closed_max_rows

                        closed_cap = _mega_panel_closed_max_rows()
                    except Exception:
                        closed_cap = 100
                    closed_ui = _sort_closed_by_exit_desc(closed_ui)[:closed_cap]
                    if not open_ui:
                        open_ui = _enrich_paper_positions_for_ui(
                            list(book.get("open") or []), mid
                        )
                    ...
                    if open_ui:
                        mega_unreal = sum(float(p.get("unrealized_pnl") or 0) for p in open_ui)
                        mega_stake = sum(float(p.get("stake_usd") or 0) for p in open_ui)
                        anchor = float(
                            summ.get("starting_capital")
                            or summ.get("session_anchor")
                            or 5000.0
                        )
                        closed_pnl = sum(
                            float(
                        ...
                        realized = float(summ.get("realized_pnl") or closed_pnl or 0)
                        equity = anchor + realized + mega_unreal
                        summ["unrealized_pnl"] = round(mega_unreal, 2)
                        summ["open_count"] = len(open_ui)
                        summ["open_trades"] = len(open_ui)
                        summ["closed_trades"] = len(closed_ui)
                        summ["realized_pnl"] = round(realized, 2)
                        summ["current_capital"] = round(equity, 2)
                        summ["balance"] = round(equity, 2)
                        summ["equity"] = round(equity, 2)
                        summ["available_capital"] = round(max(0.0, equity - mega_stake), 2)
                        summ["available_balance"] = summ["available_capital"]
                        summ["total_pnl"] = round(realized + mega_unreal, 2)
                        if anchor > 0:
                            summ["total_pnl_pct"] = round(summ["total_pnl"] / anchor * 100.0, 2)
                        summ["source"] = "mega_sim"
                    snap["summary"] = summ
                scan = dict(snap.get("scan") or {})
                scan.update(
                    _mega_scan_context_meta()
                    if light_snap
                    else _mega_scan_context()
                )
                if is_live:
                    try:
                        from elite_trader.mega_live import _mega_scan_slot_meta

                        scan.update(_mega_scan_slot_meta())
                    except Exception:
                        scan.setdefault("max_open", execution_max_open())
                else:
                    scan.setdefault("max_open", execution_max_open())
                scan.setdefault("motor_queue", len(motor_signals))
                try:
                    from elite_trader.mega_live import (
                        _mega_panel_closed_max_rows,
                        mega_closed_panel_meta,
                    )
                    ...
                    closed_cap = 100
                    closed_meta = {}
                closed_sorted = _sort_closed_by_exit_desc(closed_ui)[:closed_cap]
                if is_live:
                    sync_info = panel_exchange_sync_meta()
                return {
                    "ok": True,
                    ...
                    "exchange_account": snap.get("exchange_account") or {},
                    "exchange_sync": sync_info,
                    "scan": scan,
                    "open": open_ui,
...
@app.get("/api/hub/market")
def api_hub_market():
    """9007 → paylaşılan mainnet mark/book (9005/9006 tüketir)."""
    try:
        from elite_trader.binance_data_hub import hub_publisher_mode, publisher_snapshot

        if not hub_publisher_mode():
            return {"ok": False, "error": "hub publisher not enabled"}
        return publisher_snapshot()
    except Exception as exc:
        return {"ok": False, "error": str(exc)}
...
def _connection_live_payload() -> dict[str, Any]:
    ...
    mega_mc = _mega_status_client() if _is_mega_primary_process() else None
    if mega_mc:
    ...
    try:
        from elite_trader.binance_data_hub import hub_consumer_mode, hub_feed_status

        if hub_consumer_mode() or (
            _is_9007_process() and os.getenv("ELITE_BINANCE_HUB_PUBLISH", "0").strip() in ("1", "true")
        ):
            hs = hub_feed_status()
            hub_meta = hs.get("hub") or {}
            bt = hs.get("bookticker") or {}
            mw = hs.get("mark_ws") or {}
            ff_ok = bool(bt.get("ok"))
            ff_lag = bt.get("lag_ms")
            ff_coins = bt.get("coins", 0)
            ws_lag = mw.get("lag_ms")
            ws_source = mw.get("source", "9007_hub")
            ws_coins = mw.get("coins", 0)
            ws_health = mw.get("health", "disconnected")
    except ImportError:
        pass
    if hub_meta is None:
        try:
        ...
        try:
            ...
            mh_bt = mega_hub_bookticker_status()
            if mh_bt and mh_bt.get("health") == "ok":
                hub_lag = mh_bt.get("lag_ms")
                use_hub = not ff_ok
                if ff_lag is not None and hub_lag is not None:
                    use_hub = int(hub_lag) < int(ff_lag)
                elif hub_lag is not None:
                    use_hub = True
                if use_hub:
                    ff_ok = True
                    ff_lag = hub_lag
                    ff_coins = max(int(ff_coins or 0), int(mh_bt.get("coins") or 0))
                    ff_src = mh_bt.get("source") or "mega_hub_book"
        except Exception:
            pass
    ...
    try:
        ...
        if mh:
            ws_lag = mh.get("lag_ms")
            ws_source = mh.get("source", "mega_hub_mark_ws")
            ws_coins = mh.get("coins", 0)
            ws_health = mh.get("health", "disconnected")
    except Exception:
        pass
    with _price_cache_lock:
    ...
    last_ok_ago: float | None = None
    ip_ban = False
    risk_age: float | None = None
    try:
        from elite_trader.connection_alerts import ip_ban_active, last_ok_age_sec

        last_ok_ago = last_ok_age_sec()
        ip_ban = ip_ban_active()
    except Exception:
        pass
    try:
    ...
    if _is_mega_primary_process():
        try:
            ...
            _dbg6fe627(
                "H2",
                "binance_elite_pro.py:_connection_live_payload",
                "conn_live",
                {
                    "api_ok": api_ok,
...
@app.post("/api/panel/client-debug")
async def api_panel_client_debug(payload: dict = Body(default_factory=dict)):
    """Panel tarayıcı debug — same-origin (CORS yok)."""
    try:
        from elite_trader.mega_live import _dbg6fe627

        _dbg6fe627(
            str(payload.get("hypothesisId") or "H0"),
            str(payload.get("location") or "panel"),
...
def _connection_alerts_payload() -> dict[str, Any]:
    if (
        _is_mega_primary_process()
        and str(os.environ.get("BINANCE_ELITE_PORT", "")).strip() == "9007"
    ):
        try:
            from elite_trader.mega_live import get_mega_client, try_recover_mega_client
    ...
    try:
        ...
        from elite_trader.pipeline_alerts import merge_into_connection_alerts
        ...
        base = build_alerts(
            ...
            desk_id=str(os.environ.get("BINANCE_ELITE_PORT", "")).strip() or None,
        )
        skip_paper = _is_paper_only_port()
        if _is_9006_process():
            try:
                from elite_trader.mega_live import mega_paper_sim_only

                skip_paper = mega_paper_sim_only()
            except Exception:
                skip_paper = True
        elif _is_9005_process():
            try:
                from elite_trader.mode_registry import berserk2_paper_only

                skip_paper = berserk2_paper_only()
            except Exception:
                skip_paper = True
        return merge_into_connection_alerts(
            base,
            live=live,
            scanner=_scanner_status_dict(),
            heartbeat=_heartbeat_payload(),
            motor_candidates=_last_motor_candidates_n,
            skip_paper_mode_banner=skip_paper,
        )
    except Exception as exc:
...
def _elite_process_log_path() -> Path:
    ...
    inst = os.getenv("MEGA_INSTANCE_ID", "").strip()
    if inst:
        for name in (
            f"binance_elite_mega_{inst}_mainnet.log",
            f"binance_elite_mega_{inst}.log",
        ):
            candidate = _ROOT / "logs" / name
            if candidate.is_file():
                return candidate
        return _ROOT / "logs" / f"binance_elite_mega_{inst}_mainnet.log"
...
@app.post("/api/mega/data/reset")
def api_mega_data_reset(
    ...
    """Borsa flatten + arşiv/sil + $anchor — motor PAUSED kalır."""
    if not _is_mega_primary_process():
        raise HTTPException(status_code=404, detail="not mega primary")
    from elite_trader.mega_control import request_data_wipe

    try:
        return request_data_wipe(
            reason=str(body.get("reason") or "panel_reset")[:200],
            capital=float(body.get("capital") or 5000.0),
            close_exchange=bool(body.get("close_exchange", True)),
        )
    except Exception as exc:
...
@app.get("/api/mega/close-audit")
def api_mega_close_audit(limit: int = 50):
    """Bot kapanış audit — JSONL (pre-send, emir, settlement, kayıt)."""
    if not _is_mega_primary_process():
        raise HTTPException(status_code=404, detail="not mega primary")
    from elite_trader.mega_live import load_mega_close_audit

    rows = load_mega_close_audit(limit=max(1, min(int(limit), 500)))
