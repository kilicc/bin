...
# 9005 demo: senaryo API anahtarları .env ezmesinden sonra geri yükle
if _scenario_path.is_file() and "8300_9005" in _scenario_path.name:
    load_dotenv(_scenario_path, override=True)
...
# Senaryo 9005: kök .env STARTING_BALANCE=22000 ezmesin
if os.environ.get("BINANCE_ELITE_PORT") == "9005" or (
    _scenario_path.is_file() and "8300_9005" in _scenario_path.name
):
...
# Demo futures — config importundan ÖNCE zorunlu
...
# 9008 MEGA LIVE — ana client yalnızca fiyat/tarama; imzalı REST MEGA client'ta
if os.environ.get("BINANCE_ELITE_PORT", "").strip() == "9008" or (
    os.environ.get("MEGA_INSTANCE_ID", "").strip() == "9008"
    and os.environ.get("MEGA_LIVE_ORDERS", "0").strip().lower() in ("1", "true", "yes")
):
    os.environ["BINANCE_FUTURES_API_KEY"] = ""
    os.environ["BINANCE_FUTURES_API_SECRET"] = ""
    os.environ["BINANCE_SKIP_MAIN_AUTH"] = "1"
...
def _force_restart_position_worker(*, reason: str) -> None:
    ...
    global _last_position_price_at
...
def update_open_positions_fast() -> float:
    ...
    coins = sorted(set(live_coins) | set(paper_coins))
...
def _position_price_worker() -> None:
    ...
    while not _position_price_stop.is_set() and _position_thread_generation == gen:
        ...
        tick += 1
        iv = _position_check_interval(has_open=True)
        try:
...
def _position_price_worker_tick(tick: int, gen: int, loop_t0: float) -> None:
    ...
    has_open = has_live or has_paper
    ...
    if has_open and loop_ms > iv * 1000 * 4.0:
        _note_position_stall(
            "döngü_gecikme",
...
def _skip_main_exchange_poll() -> bool:
    """9008 MEGA LIVE — positionRisk/wallet yalnızca mega_live worker."""
    iid = os.environ.get("MEGA_INSTANCE_ID", "").strip()
    port = os.environ.get("BINANCE_ELITE_PORT", "").strip()
    live = os.environ.get("MEGA_LIVE_ORDERS", "0").strip().lower() in ("1", "true", "yes")
    return live and (iid == "9008" or port == "9008")


def _start_exchange_poll_worker() -> None:
    global _exchange_poll_thread
    if _skip_main_exchange_poll():
        return
...
def _heartbeat_payload() -> dict[str, Any]:
    ...
    try:
        ...
        paper_open = 0
    pos_iv = _position_check_interval(has_open=(paper_open + len(positions)) > 0)
    payload = {
        "pid": os.getpid(),
        "ts": time.time(),
    ...
    try:
        from elite_trader.mega_live import mega_health, mega_live_enabled
        ...
        if mega_live_enabled():
            ...
            payload["connection"] = {
                "api_ok": bool(mh.get("api_ok")),
                "api_paper": bool(mc.paper) if mc else False,
                "auth_error": getattr(mc, "_auth_error", None)
                if mc and not mh.get("api_ok")
                else None,
                "last_ok_ago_sec": mh.get("api_last_ok_ago_sec"),
                "position_risk_age_sec": mh.get("position_risk_age_sec"),
                "stale_sec": stale,
                "ip_ban": False,
            }
            try:
                ...
                payload["connection"]["ip_ban"] = ip_ban_active()
            except Exception:
                pass
    except Exception:
        pass
...
def _refresh_binance_snapshot_positions(
    ...
    *,
    ...
    if not light:
        summary["closed_trades"] = len(closed_positions)
        summary["win_count"] = len(
            [p for p in closed_positions if float(p.get("final_pnl") or 0) > 0]
        )
        summary["closed_losses"] = max(
            0, int(summary["closed_trades"]) - int(summary["win_count"])
        )
        summary["realized_pnl"] = round(
            sum(p.get("final_pnl", p.get("pnl_usd", 0)) for p in closed_positions), 2
        )
        if closed_ui:
            summary["closed_trades"] = max(int(summary.get("closed_trades") or 0), len(closed_ui))
    if _wallet_cache:
...
def _build_api_only_panel_snapshot(*, force_positions: bool = False) -> dict[str, Any]:
    ...
    win_count = len(
        [p for p in closed_ui if float(p.get("final_pnl") or p.get("net_pnl") or 0) > 0]
    )
    realized = round(
        sum(float(p.get("final_pnl") or p.get("net_pnl") or 0) for p in closed_ui), 2
    )
...
def _enrich_paper_positions_for_ui(
    ...
    for raw in open_list:
        ...
        p.setdefault("signal_source", p.get("signal_source") or f"Paper-{mode_id}")
...
def _berserk2_scan_active() -> bool:
    """BERSERK2 top10 hızlı tarama — berserk2 veya MEGA canlı motor."""
    try:
        if active_execution_mode() == "berserk2":
            return True
        if active_execution_mode() == "mega":
...
def _evaluate_berserk2_candidate_fast(
    ...
    from elite_trader.signal_paths import build_berserk2_momentum_candidate

    exec_mid = active_execution_mode()
    prof = get_profile(exec_mid) or get_profile("berserk2") or {}
    rev = build_flash_reversal_candidate(
        symbol, price, price_history, prof, rank=rank
    )
    if rev and flash_reversal_enabled(prof):
        rev["target_mode"] = exec_mid
        if rev.get("flash_reversal"):
            rev["mega_flash_reversal"] = True
            rev["flash_reversal"] = True
...
def _emit_berserk2_candidate(cand: dict[str, Any]) -> None:
        ...
        try:
            ...
            if mega_motor_active():
                ...
                if px <= 0:
                    ...
                    with _price_cache_lock:
                        px = float(
                            _price_cache.get(coin)
                            or _price_cache.get(sym)
                            or 0
                        )
                row = dict(cand)
                row["target_mode"] = "mega"
                src = str(row.get("signal_source") or "")
                if row.get("flash_reversal") or src.startswith("FlashReversal"):
                    row["mega_flash_reversal"] = True
                    row["flash_reversal"] = True
                if process_scan_candidate(row, px):
                    sym = row.get("symbol", "?")
                    print(
                        f"  ⚡ MEGA flash giriş: {sym} {row.get('type', 'LONG')} "
                        f"Δ{float(row.get('change') or 0):+.2f}%"
                    )
                return
        except Exception as exc:
            print(f"  ⚠ MEGA aday işleme: {exc}")
    ...
    if is_live_binance_motor("berserk2"):
        ...
        return
    try:
        parallel_engine.on_market_signal_for_mode(
            "berserk2", cand, float(cand.get("price") or 0)
        )
    except Exception:
        pass


def _btc_refresh_client() -> Any:
    """BTC klines — MEGA canlıda mega client; aksi halde ana client."""
    if active_execution_mode() == "mega":
        try:
...
def scan_berserk2_top10_signals() -> None:
    ...
    try:
        ...
        if not prof.get("berserk2_fast_scan_enabled", True):
            return
        schedule_btc_refresh(_btc_refresh_client())
...
def get_snapshot(*, light: bool = False) -> dict[str, Any]:
    ...
    snap: dict[str, Any] = {
        ...
        'summary': {
            ...
            'total_pnl': total_pnl,
            'total_pnl_pct': (total_pnl / session_start * 100) if session_start else 0,
...
async def lifespan(app: FastAPI):
    ...
    _start_motor_scan_worker()
    try:
        from elite_trader.mega_live import mega_live_enabled, start_mega_rest_worker

        if mega_live_enabled():
            start_mega_rest_worker()
            print("  ⚡ MEGA REST worker: çıkış + borsa kilidi + position sync")
    except Exception as exc:
        print(f"  ⚠ MEGA REST worker başlatılamadı: {exc}")
...
async def api_live(light: Optional[str] = None):
    ...
    timeout = (
        ...
        if _panel_api_only_mode() and not force_full
        else 4.0
    )
    try:
        ...
        if _panel_api_only_mode() and _last_ws_snapshot:
...
def _attach_closed_to_panel_snap(
    ...
    sm["closed_trades"] = len(closed_ui)
    sm["win_count"] = len(
        [p for p in closed_ui if float(p.get("final_pnl") or p.get("net_pnl") or 0) > 0]
    )
    sm["realized_pnl"] = round(
        sum(float(p.get("final_pnl") or p.get("net_pnl") or 0) for p in closed_ui), 2
    )
    sm["closed_losses"] = max(0, len(closed_ui) - int(sm.get("win_count") or 0))
    snap["summary"] = sm
...
def settings_restart():
    """9005 botunu yeniden başlat (açık borsa pozisyonları korunur)."""
    try:
        ...
        return {"ok": True, "restart": info, "message": f"PID {info.get('pid')}"}
    except Exception as exc:
...
def _connection_live_payload() -> dict[str, Any]:
    ...
    try:
        from elite_trader.mega_live import get_mega_client, mega_health, mega_live_enabled

        if mega_live_enabled():
            mc = get_mega_client()
            mh = mega_health()
            api_ok = bool(mh.get("api_ok"))
            api_paper = bool(mc.paper) if mc else True
            auth_error = getattr(mc, "_auth_error", None) if mc and not api_ok else None
            try:
                stale = max(12.0, _env_float("ELITE_API_STALE_SEC", 35.0))
            except ValueError:
                stale = 35.0
            ...
            try:
                ...
                pass
            ws_lag = mh.get("cache_age_ms")
            ws_coins = 0
            ws_health = "ok" if api_ok else "degraded"
            ws_source = str(mh.get("cache_source") or "mega_rest")
            with _price_cache_lock:
                rest_coins = len(_price_cache)
            rest_age_ms = int(mh.get("cache_age_ms") or 0)
            open_n = int(mh.get("open") or 0)
            return {
                "ok": True,
                "ts_ms": now_ms,
                "api_ok": api_ok,
                ...
                "position_risk_age_sec": mh.get("position_risk_age_sec"),
                "stale_sec": stale,
                "api_label": "demo-fapi MEGA",
                "source": "mega_live",
                "async_hub": {"enabled": _async_hub_enabled()},
                "coingecko": {"ok": False},
                "coinmarketcap": {"ok": False},
...
def _connection_alerts_payload() -> dict[str, Any]:
    """Panel banner — build_alerts + hafif pipeline ipucu."""
    from elite_trader.connection_alerts import build_alerts

    alert_client = client
    api_ok = _api_healthy()
    api_paper = bool(client.paper)
    auth_error = getattr(client, "_auth_error", None)
    ...
    try:
        ...
        if mega_live_enabled():
            ...
            try:
                from elite_trader import mega_live as _ml

                open_n = len(getattr(_ml, "_mega_positions", []) or [])
            except Exception:
                pass
    except Exception:
        pass
    ...
    try:
        ss = _scanner_status_dict()
        payload["pipeline"] = {
            "symbols_with_ticks": ss.get("symbols_with_ticks", 0),
            "watchlist_total": ss.get("watchlist_total")
            or ss.get("universe_tracked", 0),
        }
    except Exception:
        pass
    return payload
...
async def api_connection_alerts():
    """Panel bağlantı banner — IP ban, auth, paper mod."""
    loop = asyncio.get_running_loop()
...
def api_checkpoints_recent(limit: int = 3):
    try:
        ...
        rows = list_recent_checkpoints(max(1, min(limit, 10)))
...
def api_paper_mode_snapshot(mode_id: str, light: int = 0):
    ...
    try:
            ...
            if mega_motor_active():
                ...
                if is_live:
                    ...
                    if sync_info.get("mismatch"):
                        from elite_trader.mega_live import prune_book_positions_not_on_exchange
                        ...
                        if only_book and not only_ex:
                            ...
                            sync_info = panel_exchange_sync_meta()
                        elif only_ex and not only_book:
                            schedule_panel_exchange_sync()
                ...
                if mega_sim_enabled() and not mega_live_enabled():
                    ...
                    if book_closed:
                        try:
                            ...
                            pass
                if not closed_ui and not is_live:
                    closed_ui = _enrich_paper_positions_for_ui(book_closed, mid)
                open_ui = sorted(
                    open_ui,
                    key=lambda x: x.get("open_ts") or "",
                    reverse=True,
                )
                closed_ui = sorted(
                    closed_ui,
                    key=lambda x: x.get("close_ts") or x.get("open_ts") or "",
                    reverse=True,
                )
                summary = snap.get("summary", {})
                scan = dict(snap.get("scan") or {})
                if scan.get("motor_queue") is None:
                    scan["motor_queue"] = len(motor_signals)
                if _last_motor_at > 0:
                    scan["last_scan_age_sec"] = round(time.time() - _last_motor_at, 1)
                if _last_motor_eval_n:
                    scan["eval_last_n"] = _last_motor_eval_n
                if _last_motor_candidates_n:
                    scan["candidates_last_n"] = _last_motor_candidates_n
                out: dict[str, Any] = {
                    "ok": True,
                    "mode_id": mid,
                    "ts": datetime.utcnow().isoformat() + "Z",
                    ...
                    "light": light_snap,
                    "live": bool(snap.get("live", is_live)),
                    "paper_sim": mega_sim_enabled() and not mega_live_enabled(),
                    "scan": scan,
                ...
                return {"ok": False, "error": "MEGA motor inactive"}
        book = parallel_engine.get_universe_book(mid)
        open_raw = list(book.get("open") or [])
        closed_raw = list(book.get("closed") or [])
        open_ui = _enrich_paper_positions_for_ui(open_raw, mid)
        closed_ui = _enrich_paper_positions_for_ui(closed_raw, mid)
        open_ui = sorted(
            open_ui,
            key=lambda x: x.get("open_ts") or "",
            reverse=True,
        ...
        summary = parallel_engine.compute_paper_summary(mid)
        return {
            "ok": True,
            "mode_id": mid,
            "ts": datetime.utcnow().isoformat() + "Z",
...
def api_logs_process(tail: int = 180, offset: int = 0):
    """Panel canlı log — süreç stdout dosyasından satır tail."""
    import os
    from pathlib import Path

    root = Path(__file__).resolve().parent
    candidates = [
        os.getenv("ELITE_PROCESS_LOG"),
        os.getenv("BINANCE_ELITE_LOG"),
        str(root / "logs" / f"binance_elite_mega_{os.getenv('MEGA_INSTANCE_ID', '9008')}_live.log"),
        str(root / "logs" / f"binance_elite_{os.getenv('BINANCE_ELITE_PORT', '9008')}.log"),
    ]
    ...
    if log_path is None:
        ...
        if logs_dir.is_dir():
            matches = sorted(
                logs_dir.glob("binance_elite*.log"),
    ...
    if log_path is None or not log_path.is_file():
        return {"ok": False, "error": "log not found", "path": None, "lines": []}
    try:
    ...
    n = max(20, min(int(tail or 180), 500))
    if off <= 0:
        chunk = all_lines[-n:]
        new_off = len(all_lines)
        reset = True
    else:
        chunk = all_lines[off : off + n]
...
def api_mega_control_status():
    try:
        from elite_trader.mega_control import entry_block_reason, get_status

        st = get_status()
        st["ok"] = True
        st["entries_allowed"] = st.get("state") == "RUNNING"
        st["entry_block_reason"] = entry_block_reason()
        return st
    except Exception as exc:
        return {"ok": False, "error": str(exc), "state": "UNKNOWN"}
...
if __name__ == "__main__":
    ...
    print("━" * 80)
    uvicorn.run(app, host="0.0.0.0", port=ELITE_PORT)
