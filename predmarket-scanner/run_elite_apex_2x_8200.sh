#!/usr/bin/env bash
# Elite APEX 2×24H — port 8200 (8150–8190 dokunulmaz)
#   ./run_elite_apex_2x_8200.sh          → ön plan (8160 gibi, log terminalde)
#   ./run_elite_apex_2x_8200.sh --bg     → arka plan + logs/elite_apex_2x_8200.scanner.log
#   ./run_elite_apex_2x_8200.sh --fresh  → DB sıfırla + başlat
#   ./run_elite_apex_2x_8200.sh --stop   → durdur
#   ./run_elite_apex_2x_8200.sh --status → durum
set -euo pipefail
cd "$(dirname "$0")"
PY="${PY:-./.venv/bin/python}"
PID_DIR=".pids"
PORT=8200
DB_APEX="data/elite_apex_2x_24h.db"
STATS_JSON="data/scan_stats_elite_apex_2x_24h.json"
LOG_DASH="logs/elite_apex_2x_8200.dash.log"
LOG_SCAN="logs/elite_apex_2x_8200.scanner.log"
MODE="fg"
FRESH=0
...
for arg in "$@"; do
  case "$arg" in
    ...
    --fresh) FRESH=1 ;;
    --stop)
      echo "  APEX 8200 durduruluyor..."
      for pf in "$PID_DIR/elite_apex_2x.dash.pid" "$PID_DIR/elite_apex_2x.scanner.pid"; do
        [[ -f "$pf" ]] || continue
    ...
    --status)
      dash_pid=""; scan_pid=""
      [[ -f "$PID_DIR/elite_apex_2x.dash.pid" ]] && dash_pid=$(cat "$PID_DIR/elite_apex_2x.dash.pid")
      [[ -f "$PID_DIR/elite_apex_2x.scanner.pid" ]] && scan_pid=$(cat "$PID_DIR/elite_apex_2x.scanner.pid")
...
echo "══════════════════════════════════════════════════════════"
echo " ELITE APEX 2×24H — http://127.0.0.1:${DASHBOARD_PORT}"
...
if [[ "$MODE" == "bg" ]]; then
  ...
  echo $! >"$PID_DIR/elite_apex_2x.dash.pid"
  sleep 2
  nohup "$PY" -m elite_trader.scanner >>"$LOG_SCAN" 2>&1 &
  echo $! >"$PID_DIR/elite_apex_2x.scanner.pid"
