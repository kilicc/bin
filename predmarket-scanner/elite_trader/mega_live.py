...
def mega_instance_data_dir() -> Path:
    """9007 → data/mega_9007/; 9006 → data/mega_9006/; legacy → data/."""
...
def mega_binance_env(key: str, default: str = "") -> str:
    """9007 → MEGA_9007_*; 9006 canlı → MEGA_9006_* / MEGA_*; paper sim → anahtar yok."""
    iid = mega_instance_id()
    if iid == "9007":
...
def _mega_rest_interval_sec() -> float:
    if _mega_hub_mark_hot():
        return max(10.0, _env_float("MEGA_HUB_REST_IDLE_SEC", 18.0))
    if _mega_positions or _mega_positions_cache:
        return max(0.45, _env_float("MEGA_REST_POLL_SEC", 1.0))
    return max(1.0, _env_float("MEGA_REST_IDLE_SEC", 8.0))
...
def _mega_rest_tick(*, force_sync: bool = False) -> None:
    ...
    if (mark_hot or hub_hot) and not force_sync and not motor_stale:
        touch_mega_cache_from_hub_marks()
    elif (mark_hot or hub_hot) and not force_sync:
        touch_mega_cache_from_hub_marks()
        refresh_mega_positions_cache(
...
def run_mega_position_sync_tick(*, force: bool = False) -> None:
    ...
    if not has_open and not force:
        return
    sync_iv = max(0.8, _env_float("MEGA_REST_SYNC_SEC", 1.5))
...
def _load_mega_closed_from_disk() -> None:
    ...
    try:
        ...
        rows = [dict(r) for r in (data.get("closed") or [])]
        if rows:
        ...
        if close_index:
            filtered = [
                ...
                or (
                    ...
                    and (r.get("exchange_close_order_id") or r.get("sync_source"))
                )
...
def panel_exchange_sync_meta() -> dict[str, Any]:
    ...
    out: dict[str, Any] = {
        ...
        "mismatch": False,
        "synced": False,
        "cache_age_sec": round(_mega_positions_list_age_sec(), 2),
    }
    if not mega_live_enabled():
        return out
    mc = get_mega_client()
    if not mc or mc.paper:
        return out
    iv = max(6.0, _env_float("MEGA_PANEL_EXCHANGE_SYNC_SEC", 8.0))
    out["sync_iv_sec"] = iv
    if _mega_positions_list_age_sec() > max(30.0, iv * 3):
        out["stale"] = True
        return out
...
def _mega_cache_ttl_open_sec() -> float:
    try:
        ...
        if mega_hub_enabled() and hub_cache_hot():
            return max(0.05, _env_float("MEGA_HUB_CACHE_TTL_SEC", 0.08))
    except Exception:
        pass
    return max(0.08, _env_float("MEGA_CACHE_TTL_OPEN_SEC", 0.11))
...
def refresh_mega_positions_cache(
    *,
    ...
    max_stale = max(0.35, _env_float("MEGA_CACHE_MAX_STALE_SEC", 0.55))
    ...
    if not force:
        ...
        try:
            ...
            if mega_hub_enabled() and hub_cache_hot():
                hub_hot = True
                ttl = _mega_cache_ttl_open_sec() if _mega_positions else 2.5
